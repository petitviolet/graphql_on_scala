...
object UserDao extends Dao[User] {
  private[daos] def init(): Unit = {
    this.data ++= List(
      User(UserId("u1"), ProjectId("p1"), UserName("alice"), ZonedDateTime.now()),
      User(UserId("u2"), ProjectId("p1"), UserName("bob"), ZonedDateTime.now()),
      User(UserId("u3"), ProjectId("p2"), UserName("charlie"), ZonedDateTime.now()),
    ).map { u =>
      u.id -> u
    }
  }
